...
START-HERE.md              本手册
set-lab-ip.ps1             配 lab 网段（窗口里点「准备为 A 机 / B 机」跑的就是它）
...
"状态"那一行会直接告诉你点哪个按钮一键配好（也附着手动命令），两个角色按钮会被**禁用**——
...
LanRemote.Acceptance.exe --headless prepare-lab --lab-role <A|B>    ← 「准备为 A/B 机」按钮的命令行形态
```

两条路调用的是**完全相同**的 `HostRole` / `ClientRole`，不存在「脚本跑的是另一套逻辑」；
`prepare-lab` 与窗口按钮也走同一条路（提权 → 跑包内脚本 → 状态核验）。

> `lab-apply` / `lab-undo` 是**提升实例专用**的两个动词（`prepare-lab` 会在提权后自己调它们），
> 不要手敲：它们要求**进程本身已经是管理员**，而且参数面是故意做窄的——
> 只接受「准备成哪台机器」和日志落点，多给一个选项就报错。
...
| 3 | 配 IP 那一步要管理员：窗口里点「准备为 A 机 / B 机」会弹**一次 UAC**（不必自己开管理员 PowerShell）。**验收本身不需要提权** | 点按钮时弹出 UAC 提示 |
...
```
[INFO][RESULT] outcome=UNMET reason=no-qualified-rfc1918-nic // 在窗口里点「准备为 A 机 / B 机」即可（会弹一次 UAC）；也可以手动跑 set-lab-ip.ps1
```

（`UNMET` = 前置条件不满足，退出码 `2`。"网卡不合格"不是产品缺陷，所以它不叫 FAIL。）

### 1.2 配 lab 网段（每台机器跑一次，角色不同）

**不用自己开 PowerShell。** 双击 `LanRemote.Acceptance.exe`，窗口上半部分的那排按钮：

| 机器 | 点哪个按钮 | 结果 |
| --- | --- | --- |
| A（控制端） | `准备为 A 机（192.168.1.10）` | 本机拿到 `192.168.1.10/24` |
| B（被控端） | `准备为 B 机（192.168.1.20）` | 本机拿到 `192.168.1.20/24` |
| 任意 | `撤销准备（还原网络设置）` | 删掉 lab 地址、把网卡交还 DHCP（或恢复保存过的原配置），并删掉那两条防火墙规则；网络类别无记录时保持不动（见 §9） |

会弹**一次 UAC**。改本机网络配置必须提权，而按项目约定这类动作**绝不能静默执行**，
必须由你显式触发：主程序自己不提权，提权只发生在那个只做一件事的窄域动作里
（参数只有"准备成哪台机器"和日志落点，不接受任意脚本路径，且执行前会重校验脚本就在本 exe 目录里）。
点「是」它自己做完，**做完自动重新自检**，状态转绿、角色按钮解禁。
点「否」不会反复弹窗，日志里记一行 `uac-declined` 就收手（`本机没有任何更改`）。

窗口这条路和下面手敲命令走的是**同一份脚本**（包里的 `set-lab-ip.ps1`）、同一套动作、
同样的核验，差别只是窗口在中间加了一层提权和实时日志转发。
手动路径依然有效——窗口不方便用时（或要把脚本单独给别人）就用它：
...
配完后窗口会**自动重新自检**，"状态"应该变成绿色的 **可以参与两机验收**，两个角色按钮解禁。
（走手动路径的话，回来点一下 `重新自检` 按钮，效果一样。）
...
| 现象 | 原因 |
| --- | --- |
| 状态红字 `no-qualified-rfc1918-nic` | 本机还没进 lab 网段。点「准备为 A 机 / B 机」一键配（等价手动跑 `set-lab-ip.ps1`），配完会自动重新自检；`ipconfig` 复核 |
| 两个角色按钮是灰的 | 同上——这是**故意的**，防止点了没反应 |
| 点「准备为 …」之后 UI 里说 `uac-declined` | 你在 UAC 弹窗上点了「否」。**本机没有任何改动**，想继续就再点一次 |
...
> 网络类别（`公用` / `专用`）只在脚本保存过原类别时才会被改回去，没有记录时保持不动。
> 类别只影响 Windows 防火墙的策略档位、不影响使用（LanRemote 的规则用 `Profile Any`，
> 不依赖类别）；在意的话在「设置 > 网络和 Internet」里手动改回即可。
