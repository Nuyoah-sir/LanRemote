...
>
> ## ✅ 本关卡已通过：2026-09-20 17:30–18:18，两台实机 20/20
>
> HANDOFF 第 9 节已改为 `Two-machine manual discovery: PASS`，完整回填见 **HANDOFF 第 9.1 节**。
> 本文档保留作为**可复现的操作记录**；若之后改动了 discovery 收发逻辑，按本文档重跑即可。
>
> 对应构建：M2.1（`Last code commit 313c542`），版本 `0.1.0-m2`。
>
> **⚠ 2026-09-20 首次实测失败，根因已定位：两台机器都在 `172.100.166.x`，不是 RFC1918。**
> 第二次用 `set-lab-ip.ps1` 加 `192.168.1.x` 私有地址后通过。请先读第 1.1 / 1.2 节。
>
> **注意：`set-lab-ip.ps1` 与 `check-logs.ps1` 都必须在仓库里取最新版。**
> 首版有两个致命缺陷（会把 lab 地址加到 VMware 虚拟网卡上、以及脚本本身语法错误），
> 详见第 1.2 节与第 4 节。压缩包 `LanRemote-0.1.0-m2-win-x64.zip` 内的版本已修复。
...
### 5.1 已回填：2026-09-20 实测结果（PASS）

```text
验收日期        : 2026-09-20 17:30–18:18
机器 A 主机名/IP : DESKTOP-D132BMD / M5WC-14GX / 172.100.166.220 + lab 192.168.1.10
机器 B 主机名/IP : DESKTOP-CU2263D / 3ERD-R74V  / 172.100.166.65  + lab 192.168.1.20
子网/掩码       : 192.168.1.0/24（由 set-lab-ip.ps1 在原有 172.100.166.0/24 之上追加）
A 网卡类型       : Ethernet（Realtek PCIe GbE）
B 网卡类型       : Ethernet（Realtek Gaming 2.5GbE）+ 3 张 VMware 虚拟网卡 + 以太网 3
防火墙           : 需要（脚本自动创建 LanRemote Discovery UDP 45872，Profile Any）

步骤 1-20 结果   : 全部通过（20/20）
步骤 18 B 日志原文: 见下方 A 机侧证据 ——
   17:51:03.892 已回应来自 192.168.1.20 的 probe：unicast → 192.168.1.20:45872（不使用源端口 54670）。
   18:04:13.550 已回应来自 192.168.1.20 的 probe：unicast → 192.168.1.20:45872（不使用源端口 58869）。
check-logs  verdict（A）: PASS（-Since "2026-09-20 17:29"，Access Key matches 0）
check-logs  verdict（B）: PASS（-Since "2026-09-20 17:50"，Access Key matches 0）
崩溃            : 无（A pid 43572 连续运行 17:30→18:18；B 全程可见且可响应）
其他异常        : 见 5.2
```
...
| 现象 | 定性 | 说明 |
| --- | --- | --- |
| B 机 `-Role B` 把 `192.168.1.20` 加到了 **VMware VMnet1** 上 | **脚本缺陷**（已修） | 虚拟网卡也是 `Up` + `802.3` + 有 IPv4，被自动选中；其流量不上物理网线 → A 完全收不到。已加虚拟网卡过滤，见 1.2 |
| B 机关闭「允许被发现」期间，A 侧出现「发现 → 离线 → 发现」抖动 | **符合设计**，非 bug | 不广播时 A 只能在 probe 命中瞬间看到它，7 s TTL 后消失。恢复广播后 3 分钟 0 次离线 |
| `check-logs.ps1` 报 `VERDICT: FAIL - no private IPv4 NIC` | **脚本缺陷**（已修） | 把配 lab IP **之前**（14:40–15:57）的 5 条历史记录算进来了。加 `-Since` 后为 0 |
| `check-logs.ps1` 直接抛 ParserError | **脚本缺陷**（已修） | 第 93 行 `Write-Host (...)` 少一个右括号 |
| A 机日志某段时间长时间无新记录 | **符合设计** | `LogDrop` 有防日志 DoS 的频率限制（`count <= 8 \|\| count % 256 == 0`） |

### 5.3 未覆盖项

- 「Ethernet + Wi-Fi 同一 subnet」组合未测（本次两台均为有线）。
- 第 19 条 B 侧「无崩溃」为用户未逐字确认，由「全程可见 + 可响应」佐证。
