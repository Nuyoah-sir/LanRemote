...
> 更新时间：**2026-09-24（Control、降权及有效拒绝专项均通过；最新拒绝自然结束证据见 §18.17）**
>
...
> **下一关：仅补原清单剩余密钥/错误提示/关窗等人工项；Control、降权、拒绝均不再重跑，见§18.17。不能先推进M5。** 本轮未绕过审批、未改60秒期限或网络。§18.12/13/14保留历史事故及修复证据，当前包仍为outputs/m4-approval-ui，无须因纯记账重新下载。
...
- **下一步：只补已有GUI清单剩余项。** 用户已确认本轮双方始终正常上网、审批界面正常且核对短码；不重复要求这些确认或已通过的主流程。正常授权、降权和拒绝有效轮均已通过（§18.15–17），不再重复。仅保留原清单密钥清空、错误提示、关窗及未覆盖布局项。仍不改DHCP/IP/路由/DNS/热点/网络类别/防火墙，不执行旧Undo、不进入M5。
